        ...
        >
          {promotionalPosters.map((poster, index) => (
            <SwiperSlide key={index}>
              <div className={`relative h-full w-full bg-gradient-to-br ${poster.gradient} flex items-center justify-between overflow-hidden`}
                   >
                {/* Ambient glow layers */}
                <motion.div
                  ...
                  >
                    ...
                    >
                      {[products[0]?.img, products[1]?.img, products[2]?.img]
                        .filter(Boolean)
                        .map((thumb, i) => (
                          <img key={`thumb-${i}`} src={thumb as string} alt="thumb"
                               className="w-10 h-10 rounded-full object-cover ring-2 ring-white/70 shadow" />
                        ))}
...
<section className="py-20 text-center px-6 md:px-20 bg-gradient-to-b from-pink-50 to-white">
  ...
  >
    {[
      ...
      <motion.a
        href="https://wa.me/917722893524?text=Hi!%20I'm%20interested%20in%20your%20handmade%20products%20✨"
        target="_blank"
